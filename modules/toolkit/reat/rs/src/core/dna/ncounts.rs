...
pub struct InnerNucCounts<T: Num + Sized> {
    pub A: T,
    pub C: T,
    pub G: T,
    pub T: T,
...
impl<T: Num> InnerNucCounts<T> {
    #[allow(non_snake_case)]
    pub fn new(A: T, C: T, G: T, T: T) -> Self {
        Self { A, C, G, T }
    }
    ...
    #[inline]
    pub fn mismatches(&self, reference: Nucleotide) -> T {
        match reference {
    ...
    #[inline]
    pub fn mostfreq(&self) -> (ReqNucleotide, &T)
    where
        T: PartialOrd,
    {
...
impl<T: Num> Index<ReqNucleotide> for InnerNucCounts<T> {
    type Output = T;
...
impl<T: Num> IndexMut<ReqNucleotide> for InnerNucCounts<T> {
...
mod tests {
    ...
    #[test]
    fn add() {
        let mut a = InnerNucCounts {
            A: 0,
            C: 1,
            G: 2,
            T: 3,
        };
        let b = InnerNucCounts {
            A: 1,
            C: 2,
            G: 3,
            T: 4,
        };
        let result = InnerNucCounts {
            A: 1,
            C: 3,
            G: 5,
            T: 7,
