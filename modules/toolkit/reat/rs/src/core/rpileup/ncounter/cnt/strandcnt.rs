...
use crate::core::stranding::StrandDeducer;
use biobit_core_rs::loc::{PerOrientation, Strand};
use std::iter::zip;
...
impl<'a, R, Deductor, InnerNucCounter, Data> ReadsCollider<'a, R>
    for StrandedNucCounter<Deductor, InnerNucCounter>
where
    R: AlignedRead,
    ...
    Data: std::cmp::PartialEq,
{
    ...
    fn collide(&mut self, read: &R) {
        match self.deductor.deduce(read) {
            Strand::Forward => self.forward.collide(read),
            Strand::Reverse => self.reverse.collide(read),
        }
    }
    ...
    fn result(&'a self) -> Self::ColliderResult {
        ...
        for (f, r) in zip(&mut fwd.cnts, &mut rev.cnts) {
            debug_assert!(
                [&f.cnts, &r.cnts]
                    .iter()
                    .all(|x| x.forward.is_none() && x.reverse.is_none() && x.dual.is_some())
            );
            debug_assert!(
                [&f.coverage, &r.coverage]
                    .iter()
                    .all(|x| x.forward == 0 && x.reverse == 0)
            );
            debug_assert!(f.data == r.data);

            f.coverage = PerOrientation {
                forward: f.coverage.dual,
                reverse: r.coverage.dual,
                dual: 0,
            };
            f.cnts = PerOrientation {
                forward: f.cnts.dual,
                reverse: r.cnts.dual,
                dual: None,
            };
        }
        fwd.mapped = PerOrientation {
            forward: fwd.mapped.dual,
            reverse: rev.mapped.dual,
            dual: 0,
        };
        fwd
    }
