...
use crate::pairwise::sw::storage::AlignmentSeed;
...
impl<S: scoring::Score> Best<S> {
    pub fn new() -> Self {
        Self { best: AlignmentSeed { row: 0, col: 0, score: S::min_value() } }
    }
}
...
impl<S: scoring::Score> BestDirectionTracer for Best<S> {
    type Score = S;
...
impl<S: scoring::Score> GapTracer for Best<S> { type Score = S; }

impl<S: scoring::Score> Tracer for Best<S> { type Score = S; }


impl<S: scoring::Score> Storage for Best<S> {
    fn reset(&mut self, newrows: usize, newcols: usize) {
        self.best = AlignmentSeed { row: newrows + 1, col: newcols + 1, score: S::min_value() }
    }
