        ...
        TraceMat: traceback::TraceMat + algo::Tracer<Score=S>
{
    pub fn new(storage: Storage, tracemat: TraceMat, scoring: Scheme) -> Self {
        ...
        let tracers = Tracers { storage, tracemat, _phantom: Default::default() };
        Self { algo, scoring, tracers }
    }
    ...
    pub fn scan_up_triangle<S1, S2>(&mut self, seq1: &S1, seq2: &S2, offset: usize) -> Vec<alignment::Alignment<S>>
        where
            S1: Alignable<Symbol=Smb>,
            S2: Alignable<Symbol=Smb>
    {
