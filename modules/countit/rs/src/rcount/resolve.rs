// /// Resolution is a way to turn a list of overlaps into overlap weights.
// trait Resolution {
//     type Input<'a> where Self: 'a;
//     type Output<'b> where Self: 'b;
//
//     /// Perform a resolution step by turning input (list of overlaps or an intermediate result) into output (weights or an intermediate result).
//     fn call<'a, 'b>(&self, data: &'a mut Self::Input<'a>) -> &'b mut Self::Output<'b>;
//
//     /// Chain this resolution with another resolution.
//     fn apply<Resolver>(self, nxt: Resolver) -> Apply<Self, Resolver>
//         where
//             Self: Sized,
//             Resolver: Resolution + Sized
//     {
//         Apply { first: self, second: nxt }
//     }
// }

// /// A helper struct to chain two resolutions together.
// pub struct Apply<F: Resolution, S: Resolution> {
//     first: F,
//     second: S,
// }

// impl<F: Resolution, S: Resolution> Resolution for Apply<F, S> {
//     type Input<'a> = F::Input<'a> where F: 'a, S: 'a;
//     type Output<'b> = S::Output<'b> where F: 'b, S: 'b;
//
//     fn call<'a, 'b>(&self, data: &'a mut Self::Input<'a>) -> &'b mut Self::Output<'b> {
//         self.second.call(self.first.call(data))
//     }
// }

// /// Any function can be used as a resolution step.
// // impl<I, O, T: Fn(I) -> O> Resolution for T {
// //     type Input<'a> = I;
// //     type Output<'b> = O;
// //
// //     fn call<'a, 'b>(&self, data: &'a mut Self::Input<'a>) -> &'b mut Self::Output<'b> {
...
// //
// // from biobit.ds.gindex import Overlap
// //
